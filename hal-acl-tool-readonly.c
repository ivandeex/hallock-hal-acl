 ...
 *
 * - all ACL's granted by this tool is kept in /var/run/hald/acl-list-readonly
...
/* Each entry here represents a line in the /var/run/hald/acl-list-readonly file
...
acl_apply_changes (GSList *new_acl_list, gboolean only_update_acllist, gboolean missing_files_ok)
{
	...
	for (i = new_acl_list; i != NULL; i = g_slist_next (i)) {
		...
		if (ha->add) {
			...
			g_string_append_printf (setfacl_cmdline, "-m %c:%d:" ACCESS_RIGHT_MODE " %s",
						(ha->type == HAL_ACL_UID) ? 'u' : 'g',
						(ha->type == HAL_ACL_UID) ? ha->v.uid : ha->v.gid,
						ha->device);
	 ...
	 * /var/run/hald/acl-list-readonly, do not apply ACL's on disk". So this
...
acl_device_added (void)
{
	...
	if ((s = getenv ("HAL_PROP_ACCESS_CONTROL_GRANT_USER" "_READONLY")) != NULL) {
		char **sv;
		sv = g_strsplit (s, "\t", 0);
		afd_grant_to_uid_from_userlist (afd, sv);
		g_strfreev (sv);
	}
	if ((s = getenv ("HAL_PROP_ACCESS_CONTROL_GRANT_GROUP" "_READONLY")) != NULL) {
		char **sv;
		sv = g_strsplit (s, "\t", 0);
...
acl_reconfigure_all (void)
{
	...
	for (i = 0; udis[i] != NULL; i++) {
		...
		while (libhal_psi_has_more (&psi)) {
			...
			} else if (strcmp (key, "access_control.grant_user" "_readonly") == 0) {
				sv = libhal_psi_get_strlist (&psi);
				afd_grant_to_uid_from_userlist (afd, sv);
			} else if (strcmp (key, "access_control.grant_group" "_readonly") == 0) {
				sv = libhal_psi_get_strlist (&psi);
				afd_grant_to_gid_from_grouplist (afd, sv);
			}
			libhal_psi_next (&psi);
